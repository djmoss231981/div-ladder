...
def simulate(
    ...
    idxs   = (
        pd.Series(range(steps))
        * length
        // steps
    ).clip(0, length - 1).astype(int)
    ...
    for step in range(steps):
        ...
        for i, t in enumerate(tickers):
            ...
            dividend = (
                divs[t].iat[pos]
                if historical
                else divs[t].iat[-1] / freq_map[freq]
            )
            ...
            if (
                i == len(tickers) - 1
                and last_handling != "Reinvest in itself"
            ):
                casc, reinv = earned, 0
...
with tabs[0]:
    with st.form("create"):
        ...
        cascade = [
            st.slider(
                f"{ticks[i]}→{ticks[i+1]}", 0, 100, 100, 5,
                key=f"casc_{i}"
    ...
    if run:
        ...
        st.dataframe(df)
        st.area_chart(
            df[["Total MarketValue", "Total CumulativeDivs"]]
        )
        st.line_chart(
            df["Total PortfolioValue"]
        )
...
with tabs[1]:
    ...
    if models:
        ...
        if st.button(
            ...
            data = json.loads(
                (Path("models")/user/f"{sel}.json").read_text()
            )
            df = simulate(
                **{**data, "historical": data["mode"] == "Historical"}
            )
            st.dataframe(df)
